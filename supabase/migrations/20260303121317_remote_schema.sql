...
CREATE OR REPLACE VIEW "public"."daily_active_jobs_last_7_days" WITH ( "security_invoker" = 'on'
) AS
WITH "days" AS (
    SELECT
...
CREATE SEQUENCE IF NOT EXISTS "public"."tags_id_seq" START WITH 1
INCREMENT BY 1
NO MINVALUE
NO MAXVALUE
CACHE 1;
...
CREATE OR REPLACE VIEW "public"."weekly_applicants_last_4_weeks" WITH ( "security_invoker" = 'on'
) AS
WITH "weeks" AS (
    SELECT
...
ALTER TABLE ONLY "public"."job_listings"
    ADD CONSTRAINT "job_listings_staff_id_fkey" FOREIGN KEY ("staff_id") REFERENCES "public"."staff"("id");
...
GRANT ALL ON SEQUENCE "public"."tags_id_seq"
    TO "service_role";
