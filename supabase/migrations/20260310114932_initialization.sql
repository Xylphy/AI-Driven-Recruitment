...
ALTER TABLE "public"."applicants"
  ADD CONSTRAINT "applicants_parsed_resume_id_fkey" FOREIGN KEY (parsed_resume_id) REFERENCES public.parsed_resume(id) ON UPDATE CASCADE ON DELETE SET NULL NOT valid;

ALTER TABLE "public"."applicants" validate CONSTRAINT "applicants_parsed_resume_id_fkey";

ALTER TABLE "public"."applicants"
  ADD CONSTRAINT "applicants_score_id_fkey" FOREIGN KEY (score_id) REFERENCES public.scored_candidates(id) ON UPDATE CASCADE ON DELETE SET NULL NOT valid;

ALTER TABLE "public"."applicants" validate CONSTRAINT "applicants_score_id_fkey";

ALTER TABLE "public"."applicants"
  ADD CONSTRAINT "applicants_transcribed_id_fkey" FOREIGN KEY (transcribed_id) REFERENCES public.transcribed(id) ON UPDATE CASCADE ON DELETE SET NULL NOT valid;
...
CREATE OR REPLACE FUNCTION public.get_ai_metrics_overall_by_month(p_year integer, p_month integer)
  RETURNS TABLE(
    avg_job_fit_score double precision,
...
base AS(
  ...
  FROM
    public.scored_candidates sc
    CROSS JOIN bounds b
  WHERE
    sc.created_at >= b.from_ts
    AND sc.created_at < b.to_ts
)
SELECT
...
CREATE OR REPLACE FUNCTION public.get_top_candidates_by_job_fit(p_limit integer DEFAULT 10)
  RETURNS TABLE(
    ...
    score_id uuid,
    score_data jsonb,
    job_fit_score double precision)
  LANGUAGE sql
  STABLE
  AS $function$
  SELECT
  ...
  FROM
    public.applicants a
    JOIN public.scored_candidates sc ON sc.id = a.score_id
  ORDER BY
    coalesce((sc.score_data ->> 'job_fit_score')::double precision, 0) DESC
LIMIT p_limit;
$function$;
